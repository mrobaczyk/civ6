...
PageLayouts["HeroUnit" ] = function(page)
	...
	AddRightColumnStatBox("LOC_UI_PEDIA_TRAITS", function(s)
		...
		if (pGameHeroes ~= nil) then
			local iActionCharges:number = pGameHeroes:GetHeroClassBaseCharges(eHeroClass);
			if (iActionCharges > 0) then
				s:AddIconNumberLabel({"ICON_STATS_SPREADCHARGES", nil,"HEROES"}, iActionCharges, "LOC_HUD_UNIT_PANEL_CHARGES");
			end
		end
	...
	-- Hero Abilities
	if (pGameHeroes ~= nil) then
		local eHeroClass:number = pGameHeroes:GetUnitHeroClass(unit.Index);
		local sAbilitiesString:string = FormatHeroClassAbilitiesAndCommands(eHeroClass);
