...
-- See below for m_playerListPullData.
...
-- These have to be defined before m_playerListPullData to work correctly.
-------------------------------------------------
function IsKickPlayerValidPull(iPlayerID :number)
	if(Network.IsGameHost() and iPlayerID ~= Game.GetLocalPlayer() and not GameConfiguration.GetValue("KICK_VOTE") and PlayerConfigurations[Game.GetLocalPlayer()]:IsAlive()) then
		return true;
	end
	return false;
end
...
function IsStartKickVoteValidPull(iPlayerID : number)
	...
	and not Network.HasKickVotedStarted(iPlayerID) and not Network.HasVotedToKick(iPlayerID) and PlayerConfigurations[Game.GetLocalPlayer()]:IsAlive())then
		return true;
	end
	return false;
end
...
function OnKickVoteStarted(targetPlayerID : number, fromPlayerID : number, reason : number)
	BuildPlayerList();
	if(not PlayerConfigurations[Game.GetLocalPlayer()]:IsAlive()) then return; end
...
function OnOpenExpandedPanel()		
	...
	Controls.PlayerListPanel:SetSizeY(Controls.ChatPanel:GetSizeY());
...
function UpdatePlayerEntry(iPlayerID :number)
	...
	if(pPlayerConfig ~= nil and (pPlayerConfig:IsAlive() or (GameConfiguration.IsNetworkMultiplayer() and Network.IsPlayerConnected(iPlayerID) and pPlayerConfig:GetSlotStatus() == 4))) then
		...
		local numEntries:number = PopulatePlayerPull(iPlayerID, playerEntry.PlayerListPull, m_playerListPullData);
...
function OnPlayerListPull(iPlayerID :number, iPlayerListDataID :number)
	local playerListData = m_playerListPullData[iPlayerListDataID];
...
function OnRefresh()
	...
	if(m_isChatPanelFilled and Controls.ChatEntryPanel:GetSizeY() < Controls.ChatLogPanel:GetSizeY())then
		m_isChatPanelFilled = false;
